...
- **edit 服务端校验规则（2026-09-12 实测，拒绝时 `success:false` + `text` 明确 message）**：
  - 主题至少 3 个字符：`"主题 必须要 3个字符"`；
  - 开始时间只能是整点或半点：`"时间只能是 00 和 30分钟"`；
  - 开始时间必须严格在未来：`"无法创建过去的会议!"`——正在进行中的半点时段也不可约（16:47 提交 16:30 开始被拒），"马上开会"只能取下一个整/半点（`--asap` 已固化该策略）。
