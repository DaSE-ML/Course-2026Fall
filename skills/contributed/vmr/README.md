...
| `book` | 一键预约会议室，输出申请编号；`--wait-approval` 顺带等待批准并打印会议号/密码 |
| `status` | 列出我的会议申请与审批状态 |
| `details --id <申请编号>` | 取已批准会议的参会链接/会议号/入会密码（JSON） |
| `wait --id <申请编号>` | 退避轮询等待批准（500ms→1s→2s→5s→10s，默认至多 120 秒），批准后打印会议号/密码 |
...
- 「马上开会，开两小时」→ `book --asap --duration 120 --wait-approval`（自动取下一个整/半点开始）
...
服务端校验（2026-09-12 实测）：主题至少 3 个字符、开始时间只能是整点/半点、且必须严格在未来（正在进行中的半点时段也不可约）。CLI 已自动处理，无需操心：

- 无 `--subject` 或主题不足 3 字符 → 自动用默认主题「周X会议」（按开始日期星期几）
- 开始时间自动向上对齐到整/半点；`--duration <分钟>` 可替代 `--end`
- 「马上开会」→ `--asap --duration <分钟>`：自动取下一个整/半点（预留 90 秒提交缓冲，最坏约等 29 分钟）
- 所有自动调整都会在预约摘要中打印提示；摘要里的`有效开始`/`有效结束时间`才是最终入会时间
...
也可以在 `book` 时直接加 `--wait-approval`，或单独运行 `wait --id <申请编号>`——按 500ms→1s→2s→5s→10s 退避轮询审批（默认至多 120 秒，`--timeout` 可调），批准后直接打印会议号/密码。实测（2026-08-28）审批由系统自动完成、即时生效（提交后 1 秒内首查即"批准"），通常首次轮询即命中。
...
预期：36 个用例全部 pass。覆盖：时间策略（整/半点对齐、`--asap` 下一个可用时段、默认主题回退）、payload 构建、选项校验（size 枚举/密码规则/周期映射）、`--field` 受控字段保护、列表规范化、list 分页参数、`zoom_info` 入会信息解析、提交/删除核验与审批退避轮询逻辑。
...
- 申请提交后需管理员审批；实测多为系统自动、即时批准（2026-08-28 实测提交后 1 秒内即"批准"），`status` / `wait --id` 可查进度。若 `wait` 超时未批（个别场景转人工），先完成预约、稍后再补入会信息即可。
